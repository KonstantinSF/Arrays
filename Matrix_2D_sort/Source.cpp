...
void main()
{
	...
	cout << delimiter;
		int buffer_counter = 0;//перечисление номеров €чеек буфферного массива
		...
		buffer_counter = 0;//обнуление счетчика €чеек буфферного массива
		...
		for (int i = 0; i < ROWS * COLS; i++)    //сортировка буфферной одномерной матрицы
		{
			for (int j = i + 1; j < ROWS * COLS; j++)
			{
				...
				{
					int buffer = 0;//буфферна€ переменна€ дл€ перестановки содержимого €чеек
					buffer = Buffer[j];
					Buffer[j] = Buffer[i];
					Buffer[i] = buffer;
				}
